...
internal class CharReader(val reader: Reader) {
    ...
    fun expectText(text: String, followedBy: Set<Char>) {
        ...
        val next = peekNext()
        if (next != null && next !in followedBy) throw MalformedJSONException("Expected text in $followedBy")
    }
}
...
class Lexer(reader: Reader) {
    ...
    private fun readStringToken(): Token {
        ...
        while (true) {
            ...
            if (c == '"') break
            if (c == '\\') {
                val escaped = charReader.readNext() ?: throw MalformedJSONException("Unterminated string")
                when (escaped) {
